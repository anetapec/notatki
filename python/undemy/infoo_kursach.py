...
class CourseType(Enum):
    DAILY_CLASS = "dzienny"
    EVENING_CLASS = "wieczorowy"
    WEEKEND_CLASS = "weekendowy"
    FOR_WORKING = "dla pracujących"
...
class InfoShareAcademyParser:
    ...
    def _find_title(self):
        logo_div_marker = self.page_html.index('<h3 class ="course-name">', self.last_course_index)
        paragraph_marker_open_end = self.page_html.index('>', logo_div_marker) + len('>')
        paragraph_marker_close = self.page_html.index('</h3>', paragraph_marker_open_end)
    ...
    def _find_duration(self, title_index):
        span_open_marker_begin = self.page_html.index('<h6 class="hours">', title_index)
        span_open_marker_end = self.page_html.index('>', span_open_marker_begin) + len('>')
        span_close_marker = self.page_html.index('</h6>', span_open_marker_end)
    ...
    def _find_type(self, time_index):
        span_open_marker_begin = self.page_html.index('<h6 class="mode">', time_index)
        span_open_marker_end = self.page_html.index('>', span_open_marker_begin) + len('>')
        span_close_marker = self.page_html.index('</h6>', span_open_marker_end)
